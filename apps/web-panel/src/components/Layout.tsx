...
import { Activity, ArrowUpCircle, Bell, Box, FileClock, LayoutDashboard, Loader2, LogOut, RadioTower, Server } from "lucide-react";
      ...
      <aside className="flex w-[68px] shrink-0 flex-col border-r border-[#151826] bg-[#080a11] sm:w-[216px]">
        ...
        <div className="border-t border-[#151826] p-3 sm:p-4">
          <div className="hidden space-y-2 font-mono text-[10px] text-[#2a3050] sm:block">
            ...
            {user && (
              <div className="flex items-center justify-between rounded-lg border border-[#111420] bg-[#0c0e16] p-2">
                <div className="truncate text-[#4a5170]">{user.username}</div>
                <Pill color={user.role === "admin" ? "purple" : user.role === "operator" ? "blue" : "gray"}>{roleLabel(user.role)}</Pill>
              </div>
            )}
            {versionInfo && (
